...
- 把视频文件直接拖进任意窗口即可加载（也支持双击窗口选择文件）；一次拖入 2~3 个文件会自动依次填入各窗口（空位先填，满了则从第一个窗口替换）
- 支持常见视频格式及 H.265 / HEVC（含 .h265 / .hevc / .265 裸流）
- 所有视频共享一条进度条，拖动时同步跳转
- 支持逐帧前进 / 后退（按钮或键盘 ← →）
- 播放 / 暂停同步进行
- 「循环」开关：开启后播完自动从头开始；播完停在结尾时点「播放」也会自动从头开始
- 2 个视频时可切换为「划像对比」：拖动白色分隔线左右（或上下）对比两侧画面，两侧角标显示对应文件名
...
| 操作 | 效果 |
| --- | --- |
| 拖入视频 / 双击窗口 | 加载视频到对应窗格；一次拖入多个文件自动依次填入各窗口 |
| 空格 | 播放 / 暂停 |
| 循环按钮 | 开启 / 关闭循环播放（播完自动从头开始） |
| ← / → | 上一帧 / 下一帧 |
| 拖动 / 单击进度条 | 所有视频同步跳转（单击任意位置直接定位） |
